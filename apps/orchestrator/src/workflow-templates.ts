...
const reachableArea: WorkflowTemplate = {
  id:          "reachable-area",
  name:        "Reachable Area",
  description: "Given a starting point, transport mode, and time budget, compute the area reachable within that time.",
  ...
  steps: [
    ...
    },
    {
      ...
      domain:      "transport",
      description: "Compute the reachable polygon for the given origin, mode, and time budget.",
      input_mappings: [
        { targetField: "lat",                source: "step_output",    from: "geocode-origin.lat" },
        { targetField: "lon",                source: "step_output",    from: "geocode-origin.lon" },
        { targetField: "transport_mode",     source: "workflow_input", from: "transport_mode" },
        { targetField: "time_budget_minutes", source: "workflow_input", from: "time_budget_minutes" },
      ],
...
const itinerary: WorkflowTemplate = {
  ...
  description: "Plan an optimised route visiting multiple points of interest, with travel times between each stop.",
  ...
  input_schema: [
    ...
    },
    {
      ...
      prompt:     "What kind of places do you want to visit? (e.g. museums, parks, cinemas)",
      input_type: "text",
      required:   true,
    },
    {
  ...
  steps: [
    ...
    },
    {
      ...
      description: "Find points of interest matching the destination query near the origin.",
      input_mappings: [
        { targetField: "lat",   source: "step_output",    from: "geocode-origin.lat" },
        { targetField: "lon",   source: "step_output",    from: "geocode-origin.lon" },
        { targetField: "query", source: "workflow_input", from: "destination_query" },
      ],
      output_key: "pois",
      optional:   true,
    },
    {
      id:          "optimise-route",
      domain:      "transport",
      description: "Order the POIs to minimise total travel time (nearest-neighbour TSP).",
      input_mappings: [
        { targetField: "origin_lat",    source: "step_output",    from: "geocode-origin.lat" },
        { targetField: "origin_lon",    source: "step_output",    from: "geocode-origin.lon" },
        { targetField: "waypoints",     source: "step_output",    from: "discover-pois.rows" },
        { targetField: "transport_mode", source: "workflow_input", from: "transport_mode" },
      ],
    ...
    },
    {
      ...
      domain:      "transport",
      description: "Compute travel time and distance between each consecutive stop.",
      input_mappings: [
        { targetField: "route",          source: "step_output",    from: "optimise-route.waypoints" },
        { targetField: "transport_mode", source: "workflow_input", from: "transport_mode" },
...
const crossDomainOverlay: WorkflowTemplate = {
  ...
  description: "Fetch results from two different domains for the same area and display them as a combined spatial overlay.",
  ...
  steps: [
    ...
    },
    {
      id:          "spatial-join",
      domain:      "overlay",
      description: "Spatially join layer_a and layer_b into a combined result set.",
      input_mappings: [
        { targetField: "handle_a", source: "step_output", from: "fetch-layer-a.handle_id" },
        { targetField: "handle_b", source: "step_output", from: "fetch-layer-b.handle_id" },
      ],
...
export function findWorkflowsForIntent(text: string): WorkflowTemplate[] {
  ...
  const scored = WORKFLOW_TEMPLATES.map((w) => ({
    template: w,
    matches:  w.trigger_intents.filter((ti) => lower.includes(ti.toLowerCase())).length,
