...
describe("searchAlternativeSources — search priority", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
    process.env.OPENROUTER_API_KEY = "test-key";
  ...
  it("returns catalogue results without calling SerpAPI", async () => {
    mockSearchCatalogue.mockResolvedValue([
      {
        url: "https://data.gov.uk/dataset/crime/resource/1.csv",
        format: "csv",
    ...
    const { searchAlternativeSources } =
      await import("../agent/workflows/shadow-recovery");
    const results = await searchAlternativeSources(
      "crime",
      "Bristol",
      "GB",
      "2023-01",
    ...
    expect(mockSearchCatalogue).toHaveBeenCalledWith("crime", "GB");
    expect(mockSearchWithSerp).not.toHaveBeenCalled();
    expect(mockCrawlerRun).not.toHaveBeenCalled();
    expect(results[0].formatHint).toBe("csv");
  });
  ...
  it("falls back to SerpAPI when catalogue returns nothing", async () => {
    ...
    expect(mockSearchWithSerp).toHaveBeenCalledWith("crime Bristol", "GB");
    expect(mockCrawlerRun).not.toHaveBeenCalled();
    expect(results).toHaveLength(1);
  });

  it("falls back to StagehandCrawler when both return nothing", async () => {
    mockSearchCatalogue.mockResolvedValue([]);
    mockSearchWithSerp.mockResolvedValue([]);
    mockPageExtract.mockResolvedValue({
      sources: [
